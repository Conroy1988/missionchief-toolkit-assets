// extension/account/browser-api.mjs
function browserCall(target, method, args = [], { timeout = 1500, allowVoid = false } = {}) {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => done(Error("Browser " + method + " request timed out.")), timeout);
    try {
      if (typeof target?.[method] !== "function") throw Error("Browser " + method + " is unavailable.");
      const pending = target[method](...args, (value) => done(globalThis.chrome?.runtime?.lastError ? Error("Browser " + method + " request failed.") : null, value));
      if (pending?.then) pending.then((value) => {
        if (value !== void 0 || allowVoid) done(null, value);
      }, () => done(Error("Browser " + method + " request failed.")));
    } catch (e) {
      done(e);
    }
  });
}
function accountDeadline(promise, message, timeout = 8e3) {
  let timer;
...
// extension/account/node_modules/@supabase/supabase-js/dist/tracingRegistry.mjs
...
// extension/account/node_modules/tslib/tslib.es6.mjs
function __rest(s, e) {
  var t = {};
...
// extension/account/node_modules/@supabase/functions-js/dist/module/helper.js
...
// extension/account/node_modules/@supabase/functions-js/dist/module/types.js
...
// extension/account/node_modules/@supabase/functions-js/dist/module/FunctionsClient.js
...
// extension/account/node_modules/@supabase/postgrest-js/dist/index.mjs
...
// extension/account/node_modules/@supabase/realtime-js/dist/module/lib/websocket-factory.js
...
// extension/account/node_modules/@supabase/realtime-js/dist/module/lib/version.js
var version = "2.116.0";

// extension/account/node_modules/@supabase/realtime-js/dist/module/lib/constants.js
...
// extension/account/node_modules/@supabase/realtime-js/dist/module/lib/serializer.js
...
// extension/account/node_modules/@supabase/realtime-js/dist/module/lib/transformers.js
var PostgresTypes;
...
// extension/account/node_modules/@supabase/phoenix/priv/static/phoenix.mjs
var closure = (value) => {
...
// extension/account/node_modules/@supabase/realtime-js/dist/module/phoenix/presenceAdapter.js
...
// extension/account/node_modules/@supabase/realtime-js/dist/module/RealtimePresence.js
...
// extension/account/node_modules/@supabase/realtime-js/dist/module/lib/normalizeChannelError.js
...
// extension/account/node_modules/@supabase/realtime-js/dist/module/phoenix/channelAdapter.js
...
// extension/account/node_modules/@supabase/realtime-js/dist/module/RealtimePostgresFilterBuilder.js
...
// extension/account/node_modules/@supabase/realtime-js/dist/module/RealtimeChannel.js
...
// extension/account/node_modules/@supabase/realtime-js/dist/module/phoenix/socketAdapter.js
...
// extension/account/node_modules/@supabase/realtime-js/dist/module/RealtimeClient.js
...
// extension/account/node_modules/iceberg-js/dist/index.mjs
...
// extension/account/node_modules/@supabase/storage-js/dist/index.mjs
function _typeof2(o) {
  "@babel/helpers - typeof";
...
// extension/account/node_modules/@supabase/auth-js/dist/module/lib/version.js
var version3 = "2.116.0";

// extension/account/node_modules/@supabase/auth-js/dist/module/lib/constants.js
...
// extension/account/node_modules/@supabase/auth-js/dist/module/lib/errors.js
...
// extension/account/node_modules/@supabase/auth-js/dist/module/lib/base64url.js
...
// extension/account/node_modules/@supabase/auth-js/dist/module/lib/helpers.js
...
// extension/account/node_modules/@supabase/auth-js/dist/module/lib/fetch.js
...
// extension/account/node_modules/@supabase/auth-js/dist/module/lib/types.js
var SIGN_OUT_SCOPES = ["global", "local", "others"];

// extension/account/node_modules/@supabase/auth-js/dist/module/GoTrueAdminApi.js
...
// extension/account/node_modules/@supabase/auth-js/dist/module/lib/local-storage.js
...
// extension/account/node_modules/@supabase/auth-js/dist/module/lib/locks.js
var internals = {
  /**
   * @experimental
   */
...
// extension/account/node_modules/@supabase/auth-js/dist/module/lib/polyfills.js
...
// extension/account/node_modules/@supabase/auth-js/dist/module/lib/web3/ethereum.js
...
// extension/account/node_modules/@supabase/auth-js/dist/module/lib/webauthn.errors.js
...
// extension/account/node_modules/@supabase/auth-js/dist/module/lib/webauthn.js
...
// extension/account/node_modules/@supabase/auth-js/dist/module/GoTrueClient.js
polyfillGlobalThis();
var DEFAULT_OPTIONS = {
  url: GOTRUE_URL,
...
// extension/account/node_modules/@supabase/auth-js/dist/module/AuthClient.js
var AuthClient = GoTrueClient_default;
var AuthClient_default = AuthClient;

// extension/account/node_modules/@supabase/supabase-js/dist/index.mjs
...
// extension/account/config.json
var config_default = {
  projectId: "lanhxfrlnvtcsfqkucac",
  url: "https://lanhxfrlnvtcsfqkucac.supabase.co",
  publishableKey: "sb_publishable__YpyO8PUUaXKG5K7Vh5F-g_6-WTeNIr",
  discordCallback: "https://lanhxfrlnvtcsfqkucac.supabase.co/auth/v1/callback",
  enabled: true,
  discordClientId: "1549244199055007774",
  testRedirect: "https://kmcajnjdgofmdfhbmadcddhokbnigomb.chromiumapp.org/account",
  liveAcceptance: "pending"
};

// extension/account/tab-auth.mjs
function createTabAuth({ tabs, runtime, vault: vault2, prepare, finish, setStatus, schedule, now = Date.now, redirect }) {
  const key = "account-tab-login";
  const invoke = (method, ...args) => new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => done(Error("Browser tab request timed out.")), 15e3);
    try {
      const pending = tabs[method](...args, (value) => done(runtime.lastError ? Error("Browser tab request failed.") : null, value));
      if (pending?.then) pending.then((value) => {
        if (value !== void 0) done(null, value);
      }, () => done(Error("Browser tab request failed.")));
    } catch {
      done(Error("Browser tab request failed."));
    }
  });
  async function active() {
    const p = await vault2.getItem(key);
    if (p && now() >= p.expires) {
      await vault2.removeItem(key);
      await setStatus("local", "Sign-in expired. Start again when ready.");
      return null;
    }
    return p;
  }
  async function cancel() {
    const p = await vault2.getItem(key);
    await vault2.removeItem(key);
    if (p) await setStatus("local", "Sign-in cancelled. Your local settings are kept.");
  }
  async function receive(id, raw) {
    const p = await active();
    if (!p || p.tabId !== id || typeof raw !== "string") return;
    let url;
    try {
      url = new URL(raw);
    } catch {
    ...
    const expected = new URL(p.redirect);
    if (url.origin !== expected.origin || url.pathname !== expected.pathname) return;
    await vault2.removeItem(key);
    const returnToAccount = async () => {
      try {
        const accountUrl = runtime.getURL("account.html");
        if (tabs.query && tabs.remove) {
          const existing = (await invoke("query", {})).find(tab => {
            try { const u=new URL(tab.url); return tab.id!==id && u.href.split("?")[0]===accountUrl && u.searchParams.get("origin")===p.origin; } catch { return false; }
          });
          if (existing) {
            await invoke("update", existing.id, {active:true});
            await invoke("remove", id);
            return;
          }
        }
        await invoke("update", id, { url: runtime.getURL("account.html") + "?origin=" + encodeURIComponent(p.origin) + "&finishing=1" });
      } catch {
      }
    };
    if (url.searchParams.has("error") || url.searchParams.getAll("code").length !== 1 || !url.searchParams.get("code")) {
      await setStatus("error", "Discord did not complete sign-in. Your local settings are kept.");
      await returnToAccount();
      return;
    }
    await setStatus("signing-in", "Finishing Discord sign-in\u2026");
    await returnToAccount();
    try {
      await finish(p, url.searchParams.get("code"));
    } catch {
      await setStatus("error", "Could not finish Discord sign-in. Return to Account and try again. Your local settings are kept.");
    }
  }
  if (tabs?.onUpdated?.addListener) tabs.onUpdated.addListener((id, change, tab) => {
    const raw = change.url || tab?.pendingUrl || tab?.url;
    if (raw) schedule(() => receive(id, raw));
  });
  if (tabs?.onRemoved?.addListener) tabs.onRemoved.addListener((id) => schedule(async () => {
    const p = await active();
    if (p?.tabId === id) await cancel();
  }));
  return {
    active,
    cancel,
    receive,
    async start(origin) {
      if (!tabs?.onUpdated?.addListener || !tabs?.create || !tabs?.update) throw Error("Browser-tab sign-in is not available in this browser.");
      if (await active()) throw Error("A sign-in is already open. Finish it or cancel before starting again.");
      let tab;
      try {
        const prepared = await prepare(origin, redirect);
        tab = await invoke("create", { url: "about:blank", active: true });
        if (!Number.isInteger(tab?.id)) throw Error("The browser did not identify the sign-in tab.");
        await vault2.setItem(key, { origin, account: prepared.account, redirect, tabId: tab.id, expires: now() + 6e5 });
        await setStatus("signing-in", "Finish signing in on the Discord tab, then return here.");
        await invoke("update", tab.id, { url: prepared.url, active: true });
        return true;
      } catch {
        await vault2.removeItem(key);
        await setStatus("error", "Could not open the Discord tab. Your local settings are kept.");
        throw Error("Could not open the Discord tab.");
      }
    },
    async poll() {
      const p = await active();
      if (!p) return;
      try {
        const tab = await invoke("get", p.tabId);
        await receive(p.tabId, tab?.pendingUrl || tab?.url);
      } catch {
      }
    }
  };
...
// extension/account/vault.mjs
var opened;
function db() {
  return opened ??= new Promise((resolve, reject) => {
    const r = indexedDB.open("toolkit-private-account-v1", 1);
    r.onupgradeneeded = () => r.result.createObjectStore("values");
    r.onsuccess = () => resolve(r.result);
    r.onerror = () => reject(Error("Private account storage is unavailable."));
  });
}
async function transaction(mode, action) {
  const d = await db();
  return new Promise((resolve, reject) => {
    const t = d.transaction("values", mode), r = action(t.objectStore("values"));
    t.oncomplete = () => resolve(r.result ?? null);
    t.onerror = t.onabort = () => reject(Error("Could not save private account state."));
  });
}
var vault = { getItem: (k) => transaction("readonly", (s) => s.get(k)), setItem: (k, v) => transaction("readwrite", (s) => s.put(v, k)), removeItem: (k) => transaction("readwrite", (s) => s.delete(k)) };

// extension/account/preferences.mjs
var scalar = (v) => typeof v === "boolean" || typeof v === "number" && Number.isFinite(v) || typeof v === "string" && v.length <= 256;
var forbidden = /secret|token|webhook|credential|password|cookie|session|payment|confirm/i;
var fields = (s) => Object.fromEntries(s.split(" ").map((k) => [k, "scalar"]));
var layout = { ...fields("position panelWidth panelHeight panelHeightPx"), groupOrder: "list", controlOrder: "lists", hiddenControls: "list", panelPosition: fields("x y") };
var schema = {
  ...fields("uiTheme theme position cleanMode markerFocus missionPulse roadPriority compactDock commandBarOpen economyMode fullscreenMap tabletMode mobileMode missionChiefReskin shortcuts autoLoadAllVehicles allianceBuildingsMap missionLockAudio missionValue customVehicleBadges allianceCredits allianceCreditMinimum missionAge unitCommitment transportWatcher"),
  commandBarPrimary: "list",
  interfaceDensity: fields("desktop tablet"),
  layoutBuilder: { schema: "scalar", layouts: { desktop: layout, tablet: layout, mobile: layout } },
  themeStudio: fields("enabled name accent surface text radius opacity blur"),
  autoHideDock: fields("enabled edge"),
  inputStudio: { hotkeys: "map", gestures: fields("enabled swipeLeft swipeRight swipeUp swipeDown") },
  quickWheel: { ...fields("enabled slotCount"), actions: "list", slots: [fields("kind id")] },
  notifications: { ...fields("enabled browserEnabled preset volume"), events: "map" },
  majorIncidentFeed: fields("enabled minimumCredits"),
  stuckDetector: fields("enabled thresholdMin"),
  missionSpawn: fields("enabled"),
  resourceGap: fields("enabled radiusMi"),
  pressureBoard: fields("includeAllianceMissions timelineLoggingEnabled"),
  transportSweep: fields("delayMs maxPerRun backgroundFirst"),
  allianceCourses: fields("day shareDuration delayMs"),
  dispatchRecruitment: fields("dispatchId buildingTypeId hiringPhase personnelDesired delayMs"),
  stationIconCopier: fields("dispatchId sourceBuildingId replaceMode delayMs"),
  expansionPlanner: fields("dispatchId buildingTypeId operationKind creditBudget maxStations delayMs"),
  payoutFlash: fields("enabled threshold durationMs template soundEnabled soundVolume mode position size audioTheme showBanner mapFlash particles staticMotion animatedEffects tierIntensity finishAudio majorTier highTier eliteTier"),
  discordReport: fields("webhookName topCategories period customStart customEnd includeChart includeComparison complexity includeForecast includeRisk"),
  financialVault: fields("enabled ruleFeedEnabled retentionDays"),
  nudge: fields("x y"),
  panelPosition: fields("x y"),
  visibility: fields("allianceMissions myMissions vehicles buildings"),
  buildingVisibility: { ...fields("scope mode"), selectedTypeIds: "list" },
  quickPins: "map",
  coverage: fields("enabled radiusMi")
};
schema.bookmarks = [fields("name shortLabel lat lng zoom pinned")];
schema.profiles = [{ ...fields("name savedAt"), mapView: fields("lat lng zoom"), settings: Object.fromEntries("theme markerFocus missionPulse roadPriority visibility allianceCredits missionAge unitCommitment transportWatcher stuckDetector missionSpawn resourceGap coverage".split(" ").map((k) => [k, schema[k]])) }];
function pick(value, shape) {
  if (value == null) return value === null ? null : void 0;
  if (shape === "scalar") return scalar(value) ? value : void 0;
  if (shape === "list") return Array.isArray(value) ? value.slice(0, 100).filter(scalar) : void 0;
  if (Array.isArray(shape)) return Array.isArray(value) ? value.slice(0, 100).map((v) => pick(v, shape[0])).filter((v) => v !== void 0) : void 0;
  if (!value || typeof value !== "object" || Array.isArray(value)) return void 0;
  const out = {};
  for (const [k, v] of Object.entries(value).sort(([a], [b]) => a.localeCompare(b))) {
    if (["__proto__", "constructor", "prototype"].includes(k) || k.length > 80) continue;
    const rule = shape === "map" ? "scalar" : shape === "lists" ? "list" : shape[k];
    if (!rule || (shape === "map" || shape === "lists") && forbidden.test(k)) continue;
    const result = pick(v, rule);
    if (result !== void 0) out[k] = result;
  }
  return out;
}
function preferences(value) {
  const out = pick(value, schema) || {};
  if (JSON.stringify(out).length > 1e5) throw Error("Preferences exceed the sync size limit.");
  return out;
}
function applyPreferences(current, cloud) {
  const safe = preferences(cloud), result = structuredClone(current);
  const merge = (a, b) => {
...
// extension/account/sync.mjs
function createSync({ readLocal, writeLocal, readCloud, writeCloud, saveBase, loadBase, backup, mergeChanges: mergeChanges2 }) {
  return async function sync(scope, resolution) {
    const local = await readLocal(), before = preferences(local), storedBase = await loadBase(scope), base = storedBase ? preferences(storedBase) : null, remote = await readCloud(scope);
    if (remote && remote.schema_version !== 1) throw Error("This backup needs a newer Toolkit version.");
    const cloud = remote ? preferences(remote.settings) : null;
    const desired = resolution === "local" ? before : resolution === "cloud" && remote ? cloud : remote ? base ? mergeChanges2(cloud, base, before) : cloud : before;
    const applied = remote ? applyPreferences(local, desired) : local;
    if (remote && JSON.stringify(before) !== JSON.stringify(desired)) {
      await backup(scope, local);
      await writeLocal(local, applied);
    }
    if (!remote || JSON.stringify(cloud) !== JSON.stringify(desired)) await writeCloud(scope, desired, remote?.revision ?? 0);
    // Anchor restored settings to the actual local preference snapshot. Local-only
    // defaults are not edits and must never be uploaded on the next sync.
    await saveBase(scope, preferences(applied));
    return { restored: !!remote && JSON.stringify(before) !== JSON.stringify(desired) };
  };
}

// extension/account/service.mjs
import { STATE_KEY, mergeChanges } from "./services/settings.js";
var validOrigin = (o) => ["https://www.missionchief.co.uk", "https://missionchief.co.uk"].includes(o);
var safeFetch = (url, init = {}) => fetch(url, { ...init, signal: AbortSignal.timeout(15e3) });
import {syncDestination} from './services/destination-sync.js';
function createAccount(settings, discord) {
  const client = createClient(config_default.url, config_default.publishableKey, { auth: { storage: vault, storageKey: "discord-auth", flowType: "pkce", persistSession: true, autoRefreshToken: false, detectSessionInUrl: false }, global: { fetch: safeFetch } });
  const readAlarm = (name) => browserCall(chrome.alarms, "get", [name]).catch(() => null);
  const scheduleAlarm = (name, options) => browserCall(chrome.alarms, "create", [name, options], { allowVoid: true }).catch(() => null);
  ...
  const statusKey = "account-status";
  const setStatus = async (state, message, extra = {}) => {
    const previous = await vault.getItem(statusKey);
    return vault.setItem(statusKey, { ...state !== "local" && previous?.lastSynced ? { lastSynced: previous.lastSynced } : {}, state, message, ...extra });
  };
  async function user() {
    const { data, error } = await client.auth.getUser();
    if (error) throw Error("Reconnect Discord to continue syncing.");
    return data.user;
  }
  async function game(origin) {
    if (!validOrigin(origin)) throw Error("Open Account from MissionChief UK.");
    const r = await safeFetch(origin + "/api/credits", { credentials: "include", cache: "no-store" });
    if (!r.ok) throw Error("Sign into MissionChief UK first.");
    const a = await r.json();
    const id = String(a.user_id || "");
    if (!/^[0-9]{1,15}$/.test(id)) throw Error("Could not verify your game account.");
    return "uk:" + id;
  }
  ...
  async function local(origin) {
    const seed = await settings.load(origin);
    return JSON.parse(seed.ls[STATE_KEY] || "{}");
  }
  async function syncWebhook(u,account,restoring) {
    const key=u.id+":"+account, c=await discord.privateConfig();
    const compatible=!c.owner||c.owner===key||c.owner==='local:'+account;
    const value=compatible?{url:c.url||'',label:c.label||'Toolkit reports'}:{url:'',label:'Toolkit reports'};
    const base=await vault.getItem('discord-base:'+key);
    const {data:remote,error}=await client.from('toolkit_discord_settings').select('destination,revision').eq('user_id',u.id).eq('game_account',account).maybeSingle();
    if(error)throw Error('Toolkit webhook sync is unavailable. No saved destination was replaced.');
    const desired=await syncDestination({local:value,base,remote,restoring,
      writeCloud:async(destination,revision)=>{
        const row={user_id:u.id,game_account:account,destination,revision:revision+1};
        const q=revision?client.from('toolkit_discord_settings').update(row).eq('user_id',u.id).eq('game_account',account).eq('revision',revision):client.from('toolkit_discord_settings').insert(row);
        const {data,error}=await q.select('revision');
        if(error||data?.length!==1)throw Error('Toolkit webhook changed or could not be saved. Sync again before posting.');
      },writeLocal:value=>discord.restore(value,key),saveBase:value=>vault.setItem('discord-base:'+key,value)});
    await discord.restore(desired,key);
  }
  async function destinationOwner(origin) {
    const account=await game(origin),s=await session();
    if(!s)return 'local:'+account;
    const b=await vault.getItem('binding');
    if(!b||b.origin!==origin||b.account!==account||b.user!==s.user.id||b.restoreComplete!==true)throw Error('Finish restoring your Discord account before changing or posting to its webhook.');
    return b.user+':'+account;
  }
  async function perform(origin, resolution) {
    const current = await session();
    if (!current) return;
    busy = true;
    try {
      await setStatus("syncing", "Checking your saved preferences\u2026");
      const [u, account] = await Promise.all([user(), game(origin)]);
      const binding = await vault.getItem("binding");
      if (!binding || binding.user !== u.id || binding.account !== account || binding.origin !== origin) throw Error("Your game account changed. Open Account and reconnect Discord for this account.");
      const scope = { user: u.id, account }, key = u.id + ":" + account;
      const sync = createSync({
        readLocal: () => local(origin),
        writeLocal: async (before, after) => {
          if (await game(origin) !== account) throw Error("Game account changed during restore.");
          await settings.save(origin, [{ area: "ls", key: STATE_KEY, before: JSON.stringify(before), value: JSON.stringify(after) }]);
        },
        loadBase: () => vault.getItem("base:" + key),
        saveBase: (_, value) => vault.setItem("base:" + key, value),
        backup: (_, value) => vault.setItem("backup:" + key, { savedAt: Date.now(), state: value }),
        mergeChanges,
        readCloud: async () => {
          const { data, error } = await client.from("toolkit_settings").select("settings,revision,schema_version").eq("user_id", u.id).eq("game_account", account).maybeSingle();
          if (error) throw Error("Cloud backup is unavailable. Your settings remain on this device.");
          return data;
        },
        writeCloud: async (_, value, revision) => {
          const row = { user_id: u.id, game_account: account, schema_version: 1, settings: value, revision: revision + 1 };
          const query = revision ? client.from("toolkit_settings").update(row).eq("user_id", u.id).eq("game_account", account).eq("revision", revision) : client.from("toolkit_settings").insert(row);
          const { data, error } = await query.select("revision");
          if (error || data?.length !== 1) throw Error("Cloud settings changed or could not be saved. Your edits are kept; use Sync now to retry.");
        }
      });
      const restoring = binding.restoreComplete !== true;
      const result = await sync(scope, restoring ? "cloud" : resolution);
      await syncWebhook(u,account,restoring);
      if (restoring) await vault.setItem("binding", { ...binding, restoreComplete: true });
      await vault.setItem("pending", false);
      await setStatus("synced", result.restored ? "Saved preferences restored." : "Your preferences are up to date.", { lastSynced: Date.now(), account });
      await scheduleAlarm("toolkit-account-refresh", { delayInMinutes: 5, periodInMinutes: 5 });
    } catch (e) {
      await setStatus(/Settings changed/.test(e.message) ? "conflict" : "pending", e.message || "Sync paused. Your settings remain on this device.");
      await vault.setItem("pending", true);
    } finally {
      busy = false;
    }
  }
  const tabAuth = createTabAuth({
    ...
    vault,
    redirect: config_default.testRedirect,
    setStatus,
    schedule: (fn) => {
      void serial(fn).catch(() => {
      });
    },
    prepare: async (origin, redirect) => {
      const account = await game(origin);
      const { data, error } = await client.auth.signInWithOAuth({ provider: "discord", options: { redirectTo: redirect, skipBrowserRedirect: true } });
      if (error || !data?.url) throw Error("Could not prepare sign-in.");
      return { account, url: data.url };
    },
    finish: async (p, code) => {
      if (await game(p.origin) !== p.account) throw Error("Game account changed.");
      await vault.removeItem("binding");
      const exchange = await client.auth.exchangeCodeForSession(code);
      if (exchange.error) throw Error("Exchange failed.");
      const u = await user();
      if (await game(p.origin) !== p.account) throw Error("Game account changed.");
      await vault.setItem("binding", { origin: p.origin, account: p.account, user: u.id, restoreComplete: false });
      await perform(p.origin);
    }
  });
  async function login(origin) {
    if (await tabAuth.active()) throw Error("Finish or cancel the browser-tab sign-in first.");
    if (!chrome.identity?.launchWebAuthFlow || !chrome.identity?.getRedirectURL) throw Error("Discord sign-in is not supported by this browser yet. You can keep using local settings.");
    let stage = "verify-game";
    try {
      const account = await game(origin);
      stage = "redirect-url";
      const redirect = await chrome.identity.getRedirectURL("account");
      if (typeof redirect !== "string" || !redirect) throw Error("Missing redirect");
      if (![config_default.testRedirect, "https://lmnojpchebgcochdfnfjmnficicnaaoc.chromiumapp.org/account"].includes(redirect)) return await tabAuth.start(origin);
      stage = "save-login-state";
      await setStatus("signing-in", "Waiting for Discord\u2026");
      stage = "prepare-discord";
      const { data, error } = await client.auth.signInWithOAuth({ provider: "discord", options: { redirectTo: redirect, skipBrowserRedirect: true } });
      if (error || !data.url) throw Error("Could not start Discord sign-in.");
      stage = "open-discord";
      const end = await new Promise((resolve, reject) => {
        const pending = chrome.identity.launchWebAuthFlow({ url: data.url, interactive: true }, (value) => {
          if (chrome.runtime.lastError) reject(Error("Browser sign-in failed"));
          else resolve(value);
        });
        if (pending && typeof pending.then === "function") pending.then((value) => {
          if (value !== void 0) resolve(value);
        }, reject);
      });
      stage = "validate-discord-return";
      if (!end) throw Error("Discord sign-in was cancelled.");
      const url = new URL(end), expected = new URL(redirect);
      if (url.origin !== expected.origin || url.pathname !== expected.pathname || url.searchParams.has("error")) throw Error("Discord sign-in was not completed.");
      const code = url.searchParams.get("code");
      if (!code) throw Error("No login code was returned.");
      stage = "exchange-login";
      const exchange = await client.auth.exchangeCodeForSession(code);
      if (exchange.error) throw Error("Could not complete Discord sign-in. Try again.");
      stage = "verify-account";
      const u = await user();
      if (await game(origin) !== account) throw Error("Game account changed during sign-in.");
      await vault.setItem("binding", { origin, account, user: u.id, restoreComplete: false });
      await perform(origin);
    } catch (e) {
      const message = "Discord sign-in stopped at " + stage + " (Orion test 1.8.2.3). Your local settings are kept.";
      try {
  ...
  async function status() {
    const s = await accountDeadline(session(), "Account session check stalled. Close the Account tab and reopen it from the Toolkit. Your local settings are kept.");
    const state = await accountDeadline(vault.getItem(statusKey), "Private account storage did not respond. Restart Orion, then reopen Account. Your local settings are kept.", 4e3);
    const alarms = await Promise.all(["toolkit-account-sync", "toolkit-account-refresh"].map(readAlarm));
    const times = alarms.map((a) => a?.scheduledTime).filter(Number.isFinite);
    const binding = await accountDeadline(vault.getItem("binding"), "Account storage check timed out.", 4e3);
    const recovery = binding ? await accountDeadline(vault.getItem("backup:" + binding.user + ":" + binding.account), "Backup status check timed out.", 4e3) : null;
    const pending = await accountDeadline(tabAuth.active(), "Sign-in progress check timed out.", 4e3);
    return { ...state || { state: "local", message: "Your preferences are saved on this device." }, nextSyncAt: s && times.length ? Math.min(...times) : null, tabSignInPending: !!pending, accountBuild: "2.0.0", signedIn: !!s, displayName: s?.user?.user_metadata?.full_name || s?.user?.user_metadata?.name || "Discord account", hasRecovery: !!recovery };
  }
  async function logout() {
    await tabAuth.cancel();
    await vault.setItem("binding", null);
    await client.auth.signOut({ scope: "local" });
    await vault.removeItem("discord-auth");
    await vault.setItem("pending", false);
    await setStatus("local", "Signed out. Your settings are still on this device.");
  }
  chrome.alarms.onAlarm.addListener((alarm) => {
    if (["toolkit-account-sync", "toolkit-account-refresh"].includes(alarm.name)) void serial(async () => {
      const binding = await vault.getItem("binding");
      if (binding) await perform(binding.origin);
    });
  });
  void (async () => {
    if (!await readAlarm("toolkit-account-refresh")) await scheduleAlarm("toolkit-account-refresh", { periodInMinutes: 5 });
  })().catch(() => {
  });
  return {
    status,
    async prepareDiscord(origin){const owner=await destinationOwner(origin),c=await discord.privateConfig();if(c.owner&&c.owner!==owner)throw Error('Set the Toolkit Discord webhook in Account for this game account first.');if(!c.url)throw Error('Add your Toolkit Discord webhook under Account first.');return discord.status();},
    changed(origin) {
      return serial(async () => {
        const b = await vault.getItem("binding");
        if (b?.origin !== origin) return;
        const base = await vault.getItem("base:" + b.user + ":" + b.account);
        if (base && JSON.stringify(preferences(await local(origin))) === JSON.stringify(preferences(base))) return;
        await vault.setItem("pending", true);
        const previous = await vault.getItem(statusKey);
        if (!["conflict", "error", "pending"].includes(previous?.state)) await setStatus("queued", "Changes saved on this device. Syncing shortly\u2026");
        if (!await readAlarm("toolkit-account-sync")) await scheduleAlarm("toolkit-account-sync", { delayInMinutes: 0.5 });
      }).catch(() => {
      });
    },
    async beforeStart(origin) {
      await serial(() => perform(origin));
    },
    async open(origin) {
      if (!validOrigin(origin)) throw Error("Unsupported game.");
      if (accountOpening) return accountOpening;
      accountOpening=(async()=>{
        const url=chrome.runtime.getURL("account.html");
        const existing=(await chrome.tabs.query({})).find(tab=>{
          try {const u=new URL(tab.url);return u.href.split("?")[0]===url&&u.searchParams.get("origin")===origin;}catch{return false;}
        });
        if(existing){await chrome.tabs.update(existing.id,{active:true});if(chrome.windows?.update&&Number.isInteger(existing.windowId))await chrome.windows.update(existing.windowId,{focused:true});}
        else await chrome.tabs.create({url:url+"?origin="+encodeURIComponent(origin)});
        return true;
      })();
      try{return await accountOpening;}finally{accountOpening=null;}
    },
    async handle(message, sender) {
      const origin = new URL(sender.url).searchParams.get("origin");
      if (!validOrigin(origin)) throw Error("Open Account from the Toolkit.");
      if (message.op === "accountWebhookStatus") return serial(async()=>{const owner=await destinationOwner(origin),c=await discord.privateConfig();return {configured:!!c.url&&(!c.owner||c.owner===owner),label:(!c.owner||c.owner===owner)?c.label||'Toolkit reports':''};});
      if (message.op === "accountWebhookSave" || message.op === "accountWebhookClear") return serial(async()=>{
        const owner=await destinationOwner(origin);
        if(message.op==='accountWebhookSave')await discord.save(message.url,message.label,owner);
        else await discord.restore({url:'',label:'Toolkit reports'},owner);
        if(await session()){await perform(origin);const s=await status();if(s.state!=='synced')throw Error(s.message);}
        return true;
      });
      if (message.op === "accountWebhookTest") {await destinationOwner(origin);const c=await discord.privateConfig();if(c.owner!==await destinationOwner(origin))throw Error('Configure this account’s webhook first.');return discord.request(false);}
      if (message.op === "accountStatus") return status();
      if (message.op === "accountTabLogin") return serial(() => tabAuth.start(origin));
      if (message.op === "accountTabCancel") return serial(() => tabAuth.cancel());
      if (message.op === "accountTabCheck") return serial(async () => {
        await tabAuth.poll();
        return status();
      });
      if (message.op === "accountExport") return { format: "MissionChief Map Command Toolkit Safe Settings Backup", schema: 5, state: preferences(await local(origin)), secretsExcluded: true };
      if (message.op === "accountRecoveryExport") {
        const b = await vault.getItem("binding");
        const backup = b && await vault.getItem("backup:" + b.user + ":" + b.account);
        if (!backup) throw Error("No recovery copy is available.");
        return { format: "MissionChief Map Command Toolkit Safe Settings Backup", schema: 5, state: preferences(backup.state), secretsExcluded: true };
      }
      if (message.op === "accountLogin") return serial(() => login(origin));
      if (message.op === "accountLogout") return serial(logout);
      if (message.op === "accountKeepLocal") return serial(() => perform(origin, "local"));
      if (message.op === "accountUseCloud") return serial(() => perform(origin, "cloud"));
      if (message.op === "accountSync") return serial(() => perform(origin));
