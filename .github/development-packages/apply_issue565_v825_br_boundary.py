#!/usr/bin/env python3
"""Apply Issue #565 v8.2.5 real multi-cell FMS 5 patient-row and BR-boundary correction."""
...
"""
new_counter = r"""    function transportSweepOptionalReleaseTextWithBreaks(node) {
        if (!node) return '';
        const parts = [];
        const walk = current => {
            if (!current) return;
...
help_manifest["runtimeGuidePatch"] = "Toolkit v8.2.5 recognises MissionChief's real multi-cell FMS 5 patient rows and preserves BR boundaries so Ambulance followed by Patient text is parsed correctly."
...
candidate.update({
    ...
    "baseline": "8.2.4",
    "scope": "Issue #565 real MissionChief multi-cell FMS 5 patient-row parsing, BR-boundary preservation and browser-faithful regression",
})
...
print(json.dumps({
    ...
    "sourceLines": source_lines,
    "sourceSha256": source_hash,
    "fix": "real multi-cell FMS 5 patient row with BR boundary",
}, indent=2))
