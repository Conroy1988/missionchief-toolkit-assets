...
p=out/'manifest.json';m=json.loads(p.read_text());m.update(version='1.3.1',version_name='Extension 1.3.1 testing · Matching icon replacement');p.write_text(json.dumps(m,indent=2)+'\n')
p=out/'BUILD.json';m=json.loads(p.read_text());m.update(extensionVersion='1.3.1',channel='icon-replacement-testing',liveValidation='Matching icon uploads require live acceptance');m.pop('automatedTests',None);p.write_text(json.dumps(m,indent=2)+'\n')
for name in ['popup.html','help.html','privacy.html','release-notes.html','README.md']:
 p=out/name;p.write_text(p.read_text().replace('1.2.5','1.3.1'))
