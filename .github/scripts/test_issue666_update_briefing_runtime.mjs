...
assert.equal(metadataVersion, "10.9.2");
...
assert.match(body, /v10\.9\.2/u);
assert.match(body, /Background-first Patient Transport Sweep/u);
...
assert.equal(modal.title, "What’s New & Feature Beacon · v10.9.2");
assert.equal(modal.subtitle, "Background-first Patient Transport Sweep");
...
assert.deepEqual(opened[0].args, [
  "https://github.com/Conroy1988/missionchief-toolkit-assets/releases/tag/v10.9.2",
  "_blank",
  "noopener,noreferrer",
]);
