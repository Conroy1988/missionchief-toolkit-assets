...
assert.equal(metadataVersion, "10.10.0");
...
assert.match(body, /v10\.10\.0/u);
assert.match(body, /Building-type scoped Dispatch Recruitment/u);
...
assert.equal(modal.title, "What’s New & Feature Beacon · v10.10.0");
assert.equal(modal.subtitle, "Building-type scoped Dispatch Recruitment");
...
assert.deepEqual(opened[0].args, [
  "https://github.com/Conroy1988/missionchief-toolkit-assets/releases/tag/v10.10.0",
  "_blank",
  "noopener,noreferrer",
]);
