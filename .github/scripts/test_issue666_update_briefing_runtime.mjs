...
assert.equal(metadataVersion, "10.16.2");
...
assert.match(body, /<strong>v10\.16\.2<\/strong>/u);
assert.match(body, /Dispatch Recruitment State Isolation/u);
...
assert.equal(modal.title, "What’s New & Feature Beacon · v10.16.2");
assert.equal(modal.subtitle, "Dispatch Recruitment State Isolation");
...
assert.deepEqual(opened[0].args, [
  "https://github.com/Conroy1988/missionchief-toolkit-assets/releases/tag/v10.16.2",
  "_blank",
  "noopener,noreferrer",
]);
