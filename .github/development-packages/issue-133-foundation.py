...
TEST = ROOT / ".github" / "scripts" / "test_mission_requirements_contract.py"
...
if version_occurrences < 2:
    raise SystemExit(f"version baseline: expected at least two 4.14.10 occurrences, found {version_occurrences}")
source = source.replace("4.14.10", "4.15.0")
...
source = replace_once(
    ...
    "SCRIPT mission-requirements panel ID",
)
...
source = replace_once(
    ...
    "mission-requirements lifecycle state",
)
...
source = replace_once(
    ...
    "default mission-requirements state",
)
...
source = replace_once(
    ...
    "normalised mission-requirements state",
)

source = replace_once(
    source,
    "#${SCRIPT.missionInspectorId}, #${SCRIPT.helpCenterId}",
    "#${SCRIPT.missionInspectorId}, #${SCRIPT.missionRequirementsPanelId}, #${SCRIPT.helpCenterId}",
    "old-instance mission-requirements cleanup",
)
    ...
    function installMissionRequirementsWindows() {
        if (!missionRequirementsFeatureInstalled) {
            ...
            runtimeOnCleanup(() => {
                ...
                clearMissionRequirementsPanels();
                for (const context of transportSweepDocumentContexts()) {
                    try { context.doc.getElementById?.('mcms-mission-requirements-document-style')?.remove(); } catch (err) {}
                }
            });
        }
...
source = replace_once(
    ...
    feature_code + "    function criticalMissionValueForEntry(entry) {\n",
    "mission-requirements implementation insertion",
)
...
source = replace_once(
    ...
    "Ops mission-requirements toggle",
)
...
source = replace_once(
    ...
    "mission-requirements toggle mutation",
)
...
source = replace_once(
    ...
    "mission-requirements toggle lifecycle",
)
...
source = replace_once(
    ...
    "mission-requirements updateUI toggle",
)
...
source = replace_once(
    ...
    "mission-requirements mutation ownership ID",
)
...
source = replace_once(
    ...
    "mission-requirements mutation ownership ancestor",
)
...
source = replace_once(
    ...
    "mission-requirements boot installation",
)
...
source = replace_once(
    ...
    "mission-requirements main mutation routing",
)
...
changelog = replace_once(
    ...
    "4.15.0 changelog heading",
)
CHANGELOG.write_text(changelog, encoding="utf-8")

fixture = {
    "calculationCases": [
        {"name": "simple uncovered police requirement", "missing": 3, "enRoute": 0, "selected": 1, "stillNeeded": 3, "covered": False},
        {"name": "en-route reduction and selected coverage", "missing": 3, "enRoute": 1, "selected": 2, "stillNeeded": 2, "covered": True},
        {"name": "en-route fully covers requirement", "missing": 2, "enRoute": 2, "selected": 0, "stillNeeded": 0, "covered": True},
        {"name": "over-selection remains covered", "missing": 1, "enRoute": 0, "selected": 4, "stillNeeded": 1, "covered": True},
    ],
    "requiredAliases": [
        "Police cars",
    ...
    "requiredVehicleTypes": {
        "Police cars": [8, 12, 13, 19, 24, 25, 51, 52, 56, 82, 116],
        "Dog Support Unit (DSU)": [12, 53],
        "Ambulance": [5],
        "HEMS": [9],
        "EOD Heavy Equipment Vehicle": [112],
    },
    "layout": {
        "panelId": "mc-map-command-toolkit-mission-requirements",
        "normalFlowInsertion": "source.parentNode?.insertBefore(panel, source)",
        "forbiddenNormalPanelPositioning": ["position:fixed", "position:absolute"],
...
validator = replace_once(
    ...
    "validator mission-requirements contract path",
)
validator = replace_once(
    validator,
    ...
    "validator mission-requirements required tool",
)
validator = replace_once(
    validator,
    ...
    "validator mission-requirements execution",
)
VALIDATOR.write_text(validator, encoding="utf-8")

subprocess.run([sys.executable, str(TEST)], cwd=ROOT, check=True)
print("Applied Issue #133 live mission requirements foundation")
