...
assert.equal(metadataVersion, "10.17.1");
...
assert.match(body, /<strong>v10\.17\.1<\/strong>/u);
assert.match(body, /Fast Map Render and HUD Repair/u);
...
assert.equal(modal.title, "What’s New & Feature Beacon · v10.17.1");
assert.equal(modal.subtitle, "Fast Map Render and HUD Repair");
...
assert.deepEqual(opened[0].args, [
  "https://github.com/Conroy1988/missionchief-toolkit-assets/releases/tag/v10.17.1",
  "_blank",
  "noopener,noreferrer",
]);
