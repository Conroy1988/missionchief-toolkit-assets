...
const EXPECTED_VERSION = "10.7.0";
const EXPECTED_SHA = "d0c2738f83ae4a18c38b948cda47c86fb6d6d3aaa5f4e22b5dcdb44a05ff5c87";
const REPEATS = 25;
...
function fixtureHtml() {
  ...
  const settings = ["density-desktop", "density-tablet", ...Array.from({ length: 6 }, (_, index) => `quick-wheel-slot-${index}`), "major-incident-minimum", "coverage-radius", "alliance-credit-minimum", "transport-sweep-delay", "transport-sweep-max", "alliance-course-day", "alliance-course-duration", "alliance-course-delay", "payout-template", "resource-gap-radius", "stuck-threshold", "payout-threshold", "payout-duration", "payout-volume", "discord-webhook", "discord-name", "discord-top-categories", "discord-period", "discord-custom-start", "discord-custom-end", "discord-comparison", "discord-chart", "discord-complexity", "discord-risk", "discord-forecast", "finance-vault-enabled", "finance-vault-retention", "finance-rule-feed"];
  return `<!doctype html><html><body>
    ...
    <div id="mc-map-command-toolkit-panel">
      ...
      <div class="mcms-tabs">${["map", "missions", "alliance", "finance", "locations", "appearance", "settings"].map(key => `<button class="mcms-tab-btn" data-tab="${key}"></button>`).join("")}</div>
      ${["map", "missions", "alliance", "finance", "locations", "appearance", "settings"].map(key => `<section class="mcms-tab-panel" data-panel="${key}"><article class="mcms-command-card" data-command-search="${key}">${key}</article></section>`).join("")}
...
export async function measureWriteSuppression() {
  ...
  const sandbox = { console, globalThis: null, document: window.document, state, operationalStartupComplete: true,
    ...
    COMMAND_SECTION_META: Object.freeze({ map: { label: "Map", title: "Map Controls" }, missions: { label: "Missions", title: "Mission Operations" }, alliance: { label: "Alliance Admin", title: "Alliance Administration" }, finance: { label: "Finance", title: "Finance Command" }, locations: { label: "Locations", title: "Saved Locations" }, appearance: { label: "Appearance", title: "Appearance" }, settings: { label: "Settings", title: "Toolkit Settings" } }),
