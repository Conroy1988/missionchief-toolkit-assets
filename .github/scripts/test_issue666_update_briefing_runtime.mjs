...
assert.equal(metadataVersion, "10.5.0");
...
assert.match(body, /v10\.5\.0/u);
assert.match(body, /Unified on-demand Drawing workspace/u);
...
assert.equal(modal.title, "What’s New & Feature Beacon · v10.5.0");
assert.equal(modal.subtitle, "Unified on-demand Drawing workspace");
...
assert.deepEqual(opened[0].args, [
  "https://github.com/Conroy1988/missionchief-toolkit-assets/releases/tag/v10.5.0",
  "_blank",
  "noopener,noreferrer",
]);
