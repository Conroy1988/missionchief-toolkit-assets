#!/usr/bin/env node
// Current release title assertions must track the installed RELEASE_BRIEFING exactly.
...
assert.equal(metadataVersion, "10.16.8");
...
assert.match(body, /<strong>v10\.16\.8<\/strong>/u);
assert.match(body, /Performance and Responsive Layout Hardening/u);
...
assert.equal(modal.title, "What’s New & Feature Beacon · v10.16.8");
assert.equal(modal.subtitle, "Performance and Responsive Layout Hardening");
...
assert.deepEqual(opened[0].args, [
  "https://github.com/Conroy1988/missionchief-toolkit-assets/releases/tag/v10.16.8",
  "_blank",
  "noopener,noreferrer",
]);
