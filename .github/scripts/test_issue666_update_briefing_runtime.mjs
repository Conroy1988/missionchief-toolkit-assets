...
assert.equal(metadataVersion, "10.14.1");
...
assert.match(body, /v10\.14\.1/u);
assert.match(body, /Native level-upgrade route hotfix/u);
...
assert.equal(modal.title, "What’s New & Feature Beacon · v10.14.1");
assert.equal(modal.subtitle, "Native level-upgrade route hotfix");
...
assert.deepEqual(opened[0].args, [
  "https://github.com/Conroy1988/missionchief-toolkit-assets/releases/tag/v10.14.1",
  "_blank",
  "noopener,noreferrer",
]);
