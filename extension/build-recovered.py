...
parts=[]
for name in ['planner','catalogue','queue','native','images','ui']:
...
'''+anchor)
p.write_text(s)
m=json.loads((out/'manifest.json').read_text());m['version']='0.23.2';m['version_name']='Home Response testing · Extension 0.23.2 · Toolkit 10.18.1';(out/'manifest.json').write_text(json.dumps(m,indent=2)+'\n')
for name in ['TEST-RESULTS.json','TRANSPORT-TEST-RESULTS.json']:(out/name).unlink()
(out/'BUILD.json').write_text(json.dumps({'extensionVersion':'0.23.2','recoveredBaseline':'0.22.3','channel':'home-response-testing','storeSubmission':'not-submitted','livePurchaseValidation':'pending'},indent=2)+'\n')
p=out/'release-notes.html';p.write_text(p.read_text().replace('<main>','<main><h1>Home Response Builder · 0.23.2 testing</h1><p>Processes detailed area boundaries without the former complexity limit, supports up to 1,000 buildings, replaces spending caps with cost estimates, adds a city shortlist and optional verified building-image copying. Review road access and crew requirements. Live purchase acceptance remains pending.</p>',1))
archive=out.parent/'MissionChief-Toolkit-Extension-0.23.2-test.zip'
