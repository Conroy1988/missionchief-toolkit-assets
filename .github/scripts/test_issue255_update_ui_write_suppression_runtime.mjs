...
const FINGERPRINT_PATH = path.join(ROOT, ".github/fixtures/current-toolkit-candidate.json");
const FINGERPRINT = JSON.parse(fs.readFileSync(FINGERPRINT_PATH, "utf8")).source;
const SOURCE_PATH = path.join(ROOT, FINGERPRINT.path);
const BASELINE_PATH = path.join(ROOT, "docs/audits/issue-255/unchanged-update-ui.json");
const EXPECTED_VERSION = FINGERPRINT.version;
const EXPECTED_SHA = FINGERPRINT.sha256;
const REPEATS = 25;
...
function fixtureHtml() {
  ...
  return `<!doctype html><html><body>
    ...
    <div id="mc-map-command-toolkit-panel">
      ...
      <div class="mcms-tabs">${["map", "missions", "alliance", "dispatch", "finance", "locations", "appearance", "settings"].map(key => `<button class="mcms-tab-btn" data-tab="${key}"></button>`).join("")}</div>
      ${["map", "missions", "alliance", "dispatch", "finance", "locations", "appearance", "settings"].map(key => `<section class="mcms-tab-panel" data-panel="${key}"><article class="mcms-command-card" data-command-search="${key}">${key}</article></section>`).join("")}
