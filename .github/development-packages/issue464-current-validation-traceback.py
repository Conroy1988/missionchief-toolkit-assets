...
ROOT=Path(__file__).resolve().parents[2];SELF=Path(__file__).resolve();OUT=ROOT/'.github/diagnostics/issue464-current-validation-traceback.txt'
with tempfile.TemporaryDirectory(prefix='issue464-current-validation-') as temp:
    sandbox=Path(temp)/'repo';shutil.copytree(ROOT,sandbox,ignore=shutil.ignore_patterns('.git','__pycache__','*.pyc'))
    env=dict(os.environ);env['PYTHONDONTWRITEBYTECODE']='1'
    package=subprocess.run(['python3','.github/development-packages/issue464-complete-v5.py'],cwd=sandbox,env=env,text=True,capture_output=True)
