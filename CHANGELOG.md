...
- Keeps the v10.3.3 adaptive Desktop command surface while capping wide-screen command groups at a compact 210px track instead of distributing every spare map pixel.
...
- Retires the scheduled Greasy Fork fallback announcer so the discovery mirror can never advertise a stale Toolkit version to Discord.
- Gates every normal Discord release announcement on the live TKB install, update and metadata endpoints serving the exact same version and package bytes.
