...
Protected settings backups, encrypted full transfer and safe export support deliberate recovery. No configuration transfer is silent. Finance and SITREP reports remain manual; Patient Transport Sweep uses an iOS-safe current-mission fallback when Leaflet markers are unavailable, and completion posts automatically only when you have already saved a Discord webhook, containing aggregate outcome statistics only.
