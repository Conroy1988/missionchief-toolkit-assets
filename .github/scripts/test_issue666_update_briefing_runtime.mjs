...
assert.equal(metadataVersion, "10.4.0");
...
assert.match(body, /v10\.4\.0/u);
assert.match(body, /On-demand map measurement and incident cards/u);
...
assert.equal(modal.title, "What’s New & Feature Beacon · v10.4.0");
assert.equal(modal.subtitle, "On-demand map measurement and incident cards");
...
assert.deepEqual(opened[0].args, [
  "https://github.com/Conroy1988/missionchief-toolkit-assets/releases/tag/v10.4.0",
  "_blank",
  "noopener,noreferrer",
]);
