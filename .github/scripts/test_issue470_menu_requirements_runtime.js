...
(async()=>{
  ...
  const metadata=source.match(/^\/\/\s*@version\s+([^\s]+)\s*$/mu)?.[1];
  const runtimeVersion=source.match(/version:\s*'([^']+)'/u)?.[1];
  if(!metadata||!runtimeVersion||metadata!==runtimeVersion)throw new Error('candidate version metadata is incomplete or inconsistent');
  if(metadata!=='6.0.0')throw new Error(`Issue #470 expected v6.0.0 candidate, received ${metadata}`);
  console.log('Issue #470 menu-state, version and mission-scoped requirement recovery passed.');
