...
assert.equal(metadataVersion, "10.7.0");
...
assert.match(body, /v10\.7\.0/u);
assert.match(body, /Alliance Courses command automation/u);
...
assert.equal(modal.title, "What’s New & Feature Beacon · v10.7.0");
assert.equal(modal.subtitle, "Alliance Courses command automation");
...
assert.deepEqual(opened[0].args, [
  "https://github.com/Conroy1988/missionchief-toolkit-assets/releases/tag/v10.7.0",
  "_blank",
  "noopener,noreferrer",
]);
