...
function validate(item, path = '$') {
  ...
  if (type === 'object' && Object.prototype.toString.call(item) === '[object Object]') {
    Object.entries(item).forEach(([key, entry]) => validate(entry, path + '.' + key));
    return;
  }
...
"""
    ...
    if not isinstance(parsed, list):
        raise AssertionError("mission requirement definitions must evaluate to an array")
    return parsed
...
if dataset.get("schemaVersion") != 1 or dataset.get("locale") != "en_GB":
    raise AssertionError("UK mission requirement capability dataset is invalid")
if "// @version      4.20.5" not in source or "version: '4.20.5'" not in source:
    raise AssertionError("UK capability port requires the verified v4.20.5 source")

source = replace_once(source, "// @version      4.20.5", "// @version      4.20.6", "userscript metadata version")
...
for group, entries in (
    ("vehicles", dataset["vehicleRequirements"]),
    ("staff", dataset["staffRequirements"]),
):
    for entry in entries:
        ...
        matches = [
            ...
            and (
                definition.get("key") == entry.get("key")
                or alias_set.intersection(
                    normalise_alias(value) for value in definition.get("aliases", [])
                )
            )
        ]
        if len(matches) > 1:
            raise AssertionError(
                f"ambiguous existing definitions for {group}:{entry.get('key')}: "
                f"{[item.get('key') for item in matches]}"
            )
        if matches:
            target = matches[0]
        else:
            ...
            if not key:
                raise AssertionError(f"missing stable key for {group} requirement")
        ...
        target["types"] = merge_unique(
            [int(value) for value in target.get("types", [])],
            [int(value) for value in entry.get("types", [])],
        )
        if entry.get("equipment"):
            target["equipment"] = merge_unique(
                [str(value) for value in target.get("equipment", [])],
                [str(value) for value in entry.get("equipment", [])],
            )
...
sort_block = (
    ...
    "[...MISSION_REQUIREMENT_DEFINITIONS].sort((left, right) => "
    "Math.max(...((right.aliases && right.aliases.length) ? right.aliases : ['']).map(value => String(value).length)) - "
    "Math.max(...((left.aliases && left.aliases.length) ? left.aliases : ['']).map(value => String(value).length))));"
)
source = source[:freeze_close] + sort_block + source[freeze_close:]

parse_start, parse_end = function_bounds(
    source, "function missionRequirementsParseText(rawText, group = 'vehicles')"
)
parse_block = source[parse_start:parse_end]
if parse_block.count("MISSION_REQUIREMENT_DEFINITIONS") < 1:
    raise AssertionError("parser no longer references the requirement definition table")
...
if "v4.lss-manager.de" in source or "LSS-Manager" in source:
    raise AssertionError("runtime source must remain independent of external manager services")
...
runtime = replace_once(
    ...
    "runtime UK capability fixture import",
)
runtime_tests = r"""
for (const [group, entries] of [
    ...
    for (const entry of entries) {
        const matchingDefinitions = api.definitions.filter(definition =>
...
runtime = replace_once(
    ...
    "runtime UK capability coverage",
)
...
contract = replace_once(
    ...
    "contract UK capability fixture path",
)
contract = replace_once(
    contract,
    ...
    "contract UK capability fixture load",
)
contract = replace_once(
    contract,
    ...
    '        "const MISSION_REQUIREMENT_DEFINITIONS = Object.freeze([",\n'
    '        "const MISSION_REQUIREMENT_PARSE_DEFINITIONS = Object.freeze(",\n',
    "sorted parser definition contract",
)
contract = replace_once(
    contract,
    ...
    "contract UK capability invariants",
)
...
if SOURCE.stat().st_size > 3_000_000:
    raise AssertionError("v4.20.6 exceeds the owner-authorized 3,000,000-byte source ceiling")
if len(SOURCE.read_text(encoding="utf-8").splitlines()) > 32_000:
    raise AssertionError("v4.20.6 exceeds the 32,000-line source ceiling")

for cleanup_path in [
    ROOT / "status" / "uk-requirement-capability-diagnostic.txt",
    ROOT / "status" / "uk-requirement-capability-port.json",
]:
