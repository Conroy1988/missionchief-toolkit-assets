...
assert.equal(metadataVersion, "10.14.3");
...
assert.match(body, /v10\.14\.3/u);
assert.match(body, /Immediate level-target hotfix/u);
...
assert.equal(modal.title, "What’s New & Feature Beacon · v10.14.3");
assert.equal(modal.subtitle, "Immediate level-target hotfix");
...
assert.deepEqual(opened[0].args, [
  "https://github.com/Conroy1988/missionchief-toolkit-assets/releases/tag/v10.14.3",
  "_blank",
  "noopener,noreferrer",
]);
