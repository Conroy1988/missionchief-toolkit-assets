...
assert.equal(metadataVersion, "10.16.9");
...
assert.match(body, /<strong>v10\.16\.9<\/strong>/u);
assert.match(body, /Native Three-Station Building Filters/u);
...
assert.equal(modal.title, "What’s New & Feature Beacon · v10.16.9");
assert.equal(modal.subtitle, "Native Three-Station Building Filters");
...
assert.deepEqual(opened[0].args, [
  "https://github.com/Conroy1988/missionchief-toolkit-assets/releases/tag/v10.16.9",
  "_blank",
  "noopener,noreferrer",
]);
