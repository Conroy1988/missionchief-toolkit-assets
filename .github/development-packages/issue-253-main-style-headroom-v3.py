...
'''
SOURCE_LINE_OLD = '    source_lines = text.count("\\n") + 1\n'
SOURCE_LINE_NEW = (
...
def main() -> int:
    original_payload = ORIGINAL.read_text(encoding="utf-8")
    source_count = original_payload.count(SOURCE_LINE_OLD)
    if source_count != 1:
        raise RuntimeError(f"source-line convention replacement expected one match, found {source_count}")
    ORIGINAL.write_text(original_payload.replace(SOURCE_LINE_OLD, SOURCE_LINE_NEW, 1), encoding="utf-8")
    ...
    for path in (
        ...
        ROOT / ".github/development-packages/issue-253-v7-diagnostic.py",
        ...
        ROOT / "docs/issue-253-v7-package-diagnostic.txt",
