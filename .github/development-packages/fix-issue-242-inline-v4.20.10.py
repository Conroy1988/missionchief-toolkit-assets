...
start = source.find("if (baseline !== null && hasStatedRequirement)")
end = source.find("const fixedMinimum", start)
if start < 0 or end < 0:
    raise AssertionError(f"resolver baseline bounds missing: start={start}, end={end}")
...
if "inferredOnSite" not in legacy or "statedRequiredMin" not in legacy or "statedRequiredMax" not in legacy:
    raise AssertionError("resolver baseline segment does not contain the expected legacy state reconstruction")
...
print("Issue 242 inline v4.20.10 package applied")
