...
'''
new_hooks = '''source = re.sub(r"^\\s*if \\(state\\.missionRequirements\\) scheduleMissionRequirementsScan\\(0\\);\\s*\\n", "", source, flags=re.MULTILINE)
source = source.replace("scheduleMissionRequirementsScan(", "scheduleOperationalSuiteScan(")
source = source.replace(", #${SCRIPT.missionRequirementsPanelId}", "")
source = re.sub(r"^\\s*installMissionRequirementsWindows\\(\\);\\s*\\n", "", source, flags=re.MULTILINE)
'''
...
with tempfile.TemporaryDirectory(prefix="issue391-complete-retirement-selftest-") as temporary:
    ...
    if result.returncode != 0:
        ...
        raise SystemExit("Completed Issue #391 retirement package failed sandbox self-test")
...
print("Issue #391 retirement package completed; full sandbox self-test passed.")
