// Only the packaged Discord page can configure credentials or approve delivery.
...
export function createDiscord(storage,fetcher=fetch,now=Date.now) {
  ...
  return {
    ...
    async save(raw,label){if(busy)throw Error('Wait for the current Discord request.');busy=true;try{await storage.set({[key]:{id:crypto.randomUUID(),url:webhookUrl(raw),label:String(label||'Discord channel').slice(0,80)}});return true;}finally{busy=false;}},
