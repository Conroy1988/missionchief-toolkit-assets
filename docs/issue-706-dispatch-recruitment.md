...
3. If Personnel (Desired) differs, loads the station's native **Edit** endpoint and requires one unambiguous same-origin station form, its current native PUT or PATCH override, number input and Save control. MissionChief may return the form action with `personal_count_target_only=1` already present or leave that personnel-only flag for its AJAX caller to append; the Toolkit accepts only those two exact shapes and normalises the latter before submission. It uses the form's authenticity token when embedded, otherwise the current page's native Rails CSRF token. It then constructs a strict allow-listed payload, sends the native CSRF and AJAX headers and refuses any additional `building[...]` field.
...
- Every valid Personnel (Desired) keystroke updates saved Toolkit state immediately, including while Dispatch Centres or station matrices are loading, so another UI refresh or main-page change cannot restore an older value before blur. Hiring Phase and delay remain locally editable during a scan as well; centre, building-type and station scope controls stay locked until that scan finishes.
