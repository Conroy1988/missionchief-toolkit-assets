...
version_count = source.count("4.20.9")
if version_count < 3:
    raise AssertionError(f"source version markers: expected at least 3, found {version_count}")
...
source, count = old_inference.subn(
    "if (baseline !== null && hasStatedRequirement) { ({ selected, responding, onSite } = missionRequirementsReconcileCommittedDelta(requirement, baseline, selected, responding, onSite)); }",
    source,
    count=1,
)
if count != 1:
    raise AssertionError(f"legacy inferred-On-site block count: {count}")
...
print("Issue 242 v4.20.10 package applied")
