...
assert.equal(metadataVersion, "10.12.1");
...
assert.match(body, /v10\.12\.1/u);
assert.match(body, /Station Icon Copier rollback/u);
...
assert.equal(modal.title, "What’s New & Feature Beacon · v10.12.1");
assert.equal(modal.subtitle, "Station Icon Copier rollback");
...
assert.deepEqual(opened[0].args, [
  "https://github.com/Conroy1988/missionchief-toolkit-assets/releases/tag/v10.12.1",
  "_blank",
  "noopener,noreferrer",
]);
