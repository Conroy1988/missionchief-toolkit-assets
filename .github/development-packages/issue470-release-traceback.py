...
ROOT=Path(__file__).resolve().parents[2];SELF=Path(__file__).resolve();OUT=ROOT/'.github/diagnostics/issue470-release-traceback.txt'
with tempfile.TemporaryDirectory(prefix='issue470-release-trace-') as td:
    repo=Path(td)/'repo';shutil.copytree(ROOT,repo,ignore=shutil.ignore_patterns('.git','__pycache__','*.pyc'))
    env=dict(os.environ);env['PYTHONDONTWRITEBYTECODE']='1'
    package=subprocess.run(['python3','.github/development-packages/issue470-release.py'],cwd=repo,env=env,text=True,capture_output=True)
