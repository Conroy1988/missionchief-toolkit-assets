...
Toolkit v10.14.3 provides the manual, Credit-only Expansion & Upgrade Planner under **Dispatch** and supports MissionChief's current native expansion-page, level and vehicle-bay purchase flow.
...
- Native extension actions remain POST-only. A level or vehicle-bay action may use GET only for the exact same-origin `expand_do/credits?level=<target-index>` route. MissionChief's target is zero-indexed, so the immediate next displayed level uses the current authoritative API level as the sole query value: an API `level` of `0` admits only displayed Level 1 at `?level=0`, never the cumulative Level 2 target at `?level=1`.
...
- The current native CSRF token or complete native POST form is used. The exact immediate-level GET carries MissionChief's CSRF header; form fields unrelated to POST purchases are preserved.
