...
assert.equal(metadataVersion, "10.9.1");
...
assert.match(body, /v10\.9\.1/u);
assert.match(body, /Personal-first, opt-in pressure monitoring/u);
...
assert.equal(modal.title, "What’s New & Feature Beacon · v10.9.1");
assert.equal(modal.subtitle, "Personal-first, opt-in pressure monitoring");
...
assert.deepEqual(opened[0].args, [
  "https://github.com/Conroy1988/missionchief-toolkit-assets/releases/tag/v10.9.1",
  "_blank",
  "noopener,noreferrer",
]);
