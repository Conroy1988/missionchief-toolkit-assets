...
async function scenario(device, { cleanMode = false, fatalApplication = false, rootlessStart = false } = {}) {
  ...
  window.localStorage.setItem(STORAGE_KEY, JSON.stringify({
    ...
    updateBriefing: { enabled: false, seenVersion: "10.17.0", seenFeatures: [] },
  ...
  assert.equal(window[RUNTIME_KEY]?.version, "10.17.0", `${device}: wrong active runtime`);
...
assert.match(source, /^\/\/ @version\s+10\.17\.0$/mu);
