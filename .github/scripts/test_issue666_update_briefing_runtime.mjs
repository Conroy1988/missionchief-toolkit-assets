...
assert.equal(metadataVersion, "10.9.0");
...
assert.match(body, /v10\.9\.0/u);
assert.match(body, /Procurement Brain &amp; Operational Timeline/u);
...
assert.equal(modal.title, "What’s New & Feature Beacon · v10.9.0");
assert.equal(modal.subtitle, "Procurement Brain & Operational Timeline");
...
assert.deepEqual(opened[0].args, [
  "https://github.com/Conroy1988/missionchief-toolkit-assets/releases/tag/v10.9.0",
  "_blank",
  "noopener,noreferrer",
]);
