...
END_ANCHOR = "document.head.appendChild(style)"
...
def main() -> int:
    ...
    end_anchor = text.find(END_ANCHOR, assignment)
    if end_anchor < 0:
        fail("main style append anchor is missing")
    closing = text.rfind("`", assignment + len(ASSIGNMENT), end_anchor)
    if closing < 0:
        fail("main style template closing delimiter is missing")
    raw = text[assignment + len(ASSIGNMENT):closing]
    lines = raw.split("\n")
    ...
    if digest != fixture["nonEmptyLinesSha256"]:
        fail("ordered non-empty stylesheet lines differ from the reviewed fixture")
    if len(nonempty) != fixture["nonEmptyLineCount"]:
        fail("non-empty stylesheet line count differs from the reviewed fixture")
...
### Engineering
- Recovered source headroom by removing only blank physical lines from the existing `installMainStyles` CSS template.
- Preserved every ordered non-empty stylesheet line byte-for-byte, including all selectors, declarations, interpolations and cascade order.
- Added a permanent fixture and validation contract for the reviewed stylesheet source sequence and recovered-line target.
...
Only blank physical lines inside the existing `installMainStyles` template are removed. The ordered sequence of every non-empty source line remains byte-for-byte identical. Therefore the change does not group selectors, consolidate declarations, reorder rules, alter interpolation expressions, defer installation, or change specificity and cascade order.
...
- exact recovered line count;
- original and candidate template line counts;
- non-empty stylesheet line count;
- SHA-256 of the ordered non-empty stylesheet lines.

`test_main_style_source_headroom.py` is executed by canonical validation and the shared userscript preflight. It rejects returned blank lines, altered non-empty CSS source, inconsistent line arithmetic, a recovery below 500 lines or version drift.
...
''',
        ...
        for path in (INVENTORY_SCRIPT, COMPACTOR):
