...
assert.equal(metadataVersion, "10.5.1");
...
assert.match(body, /v10\.5\.1/u);
assert.match(body, /iOS Drawing workspace repair/u);
...
assert.equal(modal.title, "What’s New & Feature Beacon · v10.5.1");
assert.equal(modal.subtitle, "iOS Drawing workspace repair");
...
assert.deepEqual(opened[0].args, [
  "https://github.com/Conroy1988/missionchief-toolkit-assets/releases/tag/v10.5.1",
  "_blank",
  "noopener,noreferrer",
]);
