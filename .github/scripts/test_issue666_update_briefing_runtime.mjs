...
assert.equal(metadataVersion, "10.13.0");
...
assert.match(body, /v10\.13\.0/u);
assert.match(body, /Native map visibility shortcuts/u);
...
assert.equal(modal.title, "What’s New & Feature Beacon · v10.13.0");
assert.equal(modal.subtitle, "Native map visibility shortcuts");
...
assert.deepEqual(opened[0].args, [
  "https://github.com/Conroy1988/missionchief-toolkit-assets/releases/tag/v10.13.0",
  "_blank",
  "noopener,noreferrer",
]);
