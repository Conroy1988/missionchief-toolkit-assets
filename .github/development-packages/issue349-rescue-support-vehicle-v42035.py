...
anchor_index = next(index for index, entry in enumerate(vehicle_requirements) if entry.get("key") == "rescue-support-unit-or-rescue-pump")
...
source = replace_once(source, runtime_anchor, runtime_anchor + runtime_definition, "standalone Rescue Support Vehicle runtime definition")
...
capability_anchor = next(index for index, item in enumerate(capabilities) if item.get("key") == "rescue-support-unit-or-rescue-pump")
...
'''
runtime_test = replace_once(runtime_test, "for (const group of crossSourceFixture.authoritativeLabels) {", issue349_test + "for (const group of crossSourceFixture.authoritativeLabels) {", "Issue #349 runtime regression insertion")
...
env = {**os.environ, "PYTHONDWRITEBYTECODE": "1"}
