...
assert.equal(metadataVersion, "10.16.1");
...
assert.match(body, /<strong>v10\.16\.1<\/strong>/u);
assert.match(body, /Dispatch Recruitment Restoration/u);
...
assert.equal(modal.title, "What’s New & Feature Beacon · v10.16.1");
assert.equal(modal.subtitle, "Dispatch Recruitment Restoration");
...
assert.deepEqual(opened[0].args, [
  "https://github.com/Conroy1988/missionchief-toolkit-assets/releases/tag/v10.16.1",
  "_blank",
  "noopener,noreferrer",
]);
