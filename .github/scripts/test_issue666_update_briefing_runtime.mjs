...
assert.equal(metadataVersion, "10.16.0");
...
assert.match(body, /<strong>v10\.16\.0<\/strong>/u);
assert.match(body, /Building Visibility Selector/u);
...
assert.equal(modal.title, "What’s New & Feature Beacon · v10.16.0");
assert.equal(modal.subtitle, "Building Visibility Selector");
...
assert.deepEqual(opened[0].args, [
  "https://github.com/Conroy1988/missionchief-toolkit-assets/releases/tag/v10.16.0",
  "_blank",
  "noopener,noreferrer",
]);
