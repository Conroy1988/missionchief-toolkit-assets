...
const accountService=createAccount(settings,discord);
...
async function start(sender) {
  ...
  const job = (async () => {
    await accountService.beforeStart(origin).catch(()=>{});
...
chrome.runtime.onMessage.addListener((message, sender, respond) => {
  ...
  const run = async () => {
    if(sender.id===chrome.runtime.id && sender.frameId===0 && sender.url && new URL(sender.url).origin===new URL(chrome.runtime.getURL('account.html')).origin && new URL(sender.url).pathname==='/account.html')return accountService.handle(message,sender);
    ...
    if(message?.op==='whatsNewStatus'){if(sender.id!==chrome.runtime.id||!gameOrigin(sender.url)||sender.frameId!==0||!sender.tab?.id)throw Error('Unsupported update page.');return {jobs:await jobs.list(gameOrigin(sender.url))};}
    if(message?.op==='whatsNewStatus'){if(sender.id!==chrome.runtime.id||!gameOrigin(sender.url)||sender.frameId!==0||!sender.tab?.id)throw Error('Unsupported update page.');return {jobs:await jobs.list(gameOrigin(sender.url))};}
    if(message?.op==='whatsNewStatus'){if(sender.id!==chrome.runtime.id||!gameOrigin(sender.url)||sender.frameId!==0||!sender.tab?.id)throw Error('Unsupported update page.');return {jobs:await jobs.list(gameOrigin(sender.url))};}
    if(message?.op==='whatsNewStatus'){if(sender.id!==chrome.runtime.id||!gameOrigin(sender.url)||sender.frameId!==0||!sender.tab?.id)throw Error('Unsupported update page.');return {jobs:await jobs.list(gameOrigin(sender.url))};}
    if(message?.op==='whatsNewStatus'){if(sender.id!==chrome.runtime.id||!gameOrigin(sender.url)||sender.frameId!==0||!sender.tab?.id)throw Error('Unsupported update page.');return {jobs:await jobs.list(gameOrigin(sender.url))};}
    ...
    if(['discordPost','discordDraft','discordReceipt'].includes(message.op)){
      ...
      const destination=await accountService.prepareDiscord(pageSender(sender).origin);
      const id=await discord.stage(message.payload,message.chart,{scope,kind:message.kind,generatedAt:Number.isFinite(generatedAt)&&generatedAt>0&&generatedAt<=Date.now()?generatedAt:Date.now()});
      if(message.op==='discordPost')return discord.request(true,id,destination.destinationId);
      return id;
    }
    if(message.op==='accountOpen')return accountService.open(pageSender(sender).origin);
    if(message.op==='accountSummary'){const s=await accountService.status();return {signedIn:s.signedIn,state:s.state,message:s.message,nextSyncAt:s.nextSyncAt,lastSynced:s.lastSynced};}
    if(message.op==='discordOpen')return accountService.open(pageSender(sender).origin);
    ...
    if (message.op === 'saveBatch') {const origin=pageSender(sender).origin;const receipt=await settings.save(origin,message.writes);accountService.changed(origin);return receipt;}
