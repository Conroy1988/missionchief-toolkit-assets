...
anchor="choose.append(el('h3','1. Choose what to include'));";assert s.count(anchor)==1;s=s.replace(anchor,anchor+"if(definition[0]==='station-icon-copier'){const switchEntry=el('section',null,'mcms-switch-icon-entry');switchEntry.append(button('Switch Icon',()=>IconReplacement.openIconReplacement()),el('p','Select all buildings using one icon and change them to another. Scan, choose your two icons, then preview the matches.'),el('h3','Copy Station Icons'));choose.prepend(switchEntry);}")
...
p=out/'manifest.json';m=json.loads(p.read_text());m.update(version='1.3.2',version_name='Extension 1.3.2 testing · Matching icon replacement');p.write_text(json.dumps(m,indent=2)+'\n')
p=out/'BUILD.json';m=json.loads(p.read_text());m.update(extensionVersion='1.3.2',channel='icon-replacement-testing',liveValidation='Matching icon uploads require live acceptance');m.pop('automatedTests',None);p.write_text(json.dumps(m,indent=2)+'\n')
for name in ['popup.html','help.html','privacy.html','release-notes.html','README.md']:
 p=out/name;p.write_text(p.read_text().replace('1.2.5','1.3.2'))
p=out/'help.html';p.write_text(p.read_text().replace('<main>','<main><h2>Replace matching station icons</h2><p>In Operations, open Copy station icons, then Switch Icon. Choose a building type and dispatch scope, scan custom icons, choose the original and replacement, and preview the matches. Review the selected buildings before confirming. Saved progress supports Pause and Review &amp; resume. Default icons and unreadable images are excluded. An uncertain upload is verified without automatic replay.</p>',1))
