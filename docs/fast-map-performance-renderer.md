...
The OpenFreeMap style is declarative map data, not executable Toolkit code. Its expected vector-source topology is checked before operational layers mount. Marker graphics reuse the Station Icon Copier’s audited PNG/JPEG boundary: at most 4 MB and 200 pixels at source, then at most 96 pixels in the Fast Map sprite. Cross-origin fetches omit credentials; only MissionChief’s already approved storage host can use the userscript request fallback. One failed image remains a coloured fallback point and does not stop Fast Map. An unavailable engine request, unexpected HTTP status, byte mismatch, hash mismatch, blocked verified evaluation, unavailable or incomplete base style, unsupported WebGL, startup timeout, invalid adapter, cancelled startup, route change or fatal renderer error uses the same restoration path. Fast Map never writes an enabled preference to saved Toolkit state, so a reload always starts with native MissionChief rendering.
