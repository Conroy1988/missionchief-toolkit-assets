...
assert.equal(metadataVersion, "10.18.0");
...
assert.match(body, /<strong>v10\.18\.0<\/strong>/u);
assert.match(body, /Command Interface Overhaul/u);
...
assert.equal(modal.title, "What’s New & Feature Beacon · v10.18.0");
assert.equal(modal.subtitle, "Command Interface Overhaul");
...
assert.deepEqual(opened[0].args, [
  "https://github.com/Conroy1988/missionchief-toolkit-assets/releases/tag/v10.18.0",
  "_blank",
  "noopener,noreferrer",
]);
