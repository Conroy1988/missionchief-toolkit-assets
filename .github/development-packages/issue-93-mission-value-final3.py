...
startup_start = source.index("    async function runDeferredOperationalStartup() {")
startup_end = source.index("\n    function ", startup_start + 10)
startup_anchor = "        installRadioMessageHook();\n"
startup_variant = "        installRadioMessageHook( );\n"
source = scope_shared_anchor(source, startup_anchor, startup_variant, startup_start, startup_end, 6)
...
try:
    ...
    final_source = final_source.replace(refresh_variant, refresh_anchor).replace(ui_variant, ui_anchor).replace(startup_variant, startup_anchor)
    if refresh_variant in final_source or ui_variant in final_source or startup_variant in final_source:
    ...
    for name in [
        ...
        "issue-93-mission-value-final2.py",
    ]:
        Path(__file__).with_name(name).unlink(missing_ok=True)
    for name in ["issue-93-package-error.txt", "issue-93-final-package-error.txt", "issue-93-final2-package-error.txt"]:
