...
DIAGNOSTICS = (
    ROOT / '.github/diagnostics/issue458-performance-apply-validator.txt',
    ROOT / '.github/diagnostics/issue458-final-validator.txt',
)
text = SOURCE.read_text(encoding='utf-8')
if '// @version      5.0.5' not in text:
    raise SystemExit('Final Issue #458 package requires v5.0.5')
old = '''        const rawHtml = String(carrier?.getAttribute?.('data-raw-html') || '').trim();
...
new = '''        const rawHtml = String(carrier?.getAttribute?.('data-raw-html') || '').trim();
        ...
        return fragment;'''
if text.count(old) != 1:
    raise SystemExit(f'Expected one LSSM raw innerHTML parser with createElement guard, found {text.count(old)}')
...
assertion = "if(!source.includes('if (!rawHtml || !doc?.createRange) return null;')||!source.includes('createContextualFragment?.(rawHtml)')||source.includes('holder.innerHTML = rawHtml')||source.includes('if (!rawHtml || !doc?.createElement) return null;'))throw new Error('LSSM raw parser dependency or performance contract is invalid');\n"
...
print(json.dumps({'version':'5.0.5','sha256':fixture['candidateSourceSha256'],'parserDependency':'createRange','innerHtmlParser':'createContextualFragment','diagnosticsRemoved':True}, indent=2))
