...
assert "runtimeSetTimeout(()=>{if(state.missionAge)scheduleMissionAgeRefresh(0);},250)" not in text
...
assert meta==runtime
assert tuple(int(part) for part in meta.split('.')[:3]) >= (6,0,0)
assert len(text.splitlines())<=26000
print('Issue #464 complete launcher, typed settings, runtime and Mission Age contract passed for v6.')
