...
if '// @version      5.0.5' not in text:
    raise SystemExit('Issue #458 performance package requires v5.0.5')
...
new_doc = "querySelector(selector){return selector.includes('#mission-form')?this.missionRoot:null;},createRange(){return{createContextualFragment(html){const node=new Node({visible:false});node.ownerDocument=value;node.innerHTML=html;return node;}}}};"
...
test = test.replace(anchor, anchor + "if(!source.includes('createContextualFragment?.(rawHtml)')||source.includes('holder.innerHTML = rawHtml'))throw new Error('LSSM raw parser exceeds the innerHTML performance budget');\n", 1)
...
print(json.dumps({'version':'5.0.5','sha256':fixture['candidateSourceSha256'],'innerHtmlParser':'createContextualFragment'}, indent=2))
