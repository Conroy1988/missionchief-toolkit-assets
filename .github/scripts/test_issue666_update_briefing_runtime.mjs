#!/usr/bin/env node
// v10.6.2 release title assertions must track the installed RELEASE_BRIEFING exactly.
...
assert.equal(metadataVersion, "10.6.2");
...
assert.match(body, /v10\.6\.2/u);
assert.match(body, /iOS Patient Transport Sweep parity repair/u);
...
assert.equal(modal.title, "What’s New & Feature Beacon · v10.6.2");
assert.equal(modal.subtitle, "iOS Patient Transport Sweep parity repair");
...
assert.deepEqual(opened[0].args, [
  "https://github.com/Conroy1988/missionchief-toolkit-assets/releases/tag/v10.6.2",
  "_blank",
  "noopener,noreferrer",
]);
