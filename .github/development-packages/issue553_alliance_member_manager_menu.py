#!/usr/bin/env python3
"""Apply Issue #553 v8.1.1 Alliance Member Manager menu hotfix."""
...
def replace_once(path: Path, old: str, new: str) -> None:
    ...
    if count != 1:
        raise RuntimeError(f"{path.relative_to(ROOT)} expected one replacement target, found {count}")
...
help_manifest["runtimeGuidePatch"] = (
    "Toolkit v8.1.1 restores the Alliance Operations menu section and Alliance Member "
    "Manager toggle by supporting the live rendered Alliance Map Blocker card while "
    "preserving the v8.1.0 member-list controls and lifecycle boundaries."
)
...
candidate.update(
    {
        ...
        "scope": (
            "Issue #553 Alliance Member Manager Tools-menu discovery hotfix with "
            "attribute-first and exact rendered-label fallback plus executable live-menu fixtures"
        ),
    }
)
...
print("Issue #553 v8.1.1 Alliance Member Manager menu hotfix applied and validated.")
