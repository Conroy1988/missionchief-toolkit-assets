...
assert.equal(metadataVersion, "10.12.0");
...
assert.match(body, /v10\.12\.0/u);
assert.match(body, /Station Icon consistency manager/u);
...
assert.equal(modal.title, "What’s New & Feature Beacon · v10.12.0");
assert.equal(modal.subtitle, "Station Icon consistency manager");
...
assert.deepEqual(opened[0].args, [
  "https://github.com/Conroy1988/missionchief-toolkit-assets/releases/tag/v10.12.0",
  "_blank",
  "noopener,noreferrer",
]);
