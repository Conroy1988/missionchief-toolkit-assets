...
assert.equal(metadataVersion, "10.3.9");
...
assert.match(body, /v10\.3\.9/u);
assert.match(body, /Stable map colours during movement/u);
for (const highlight of RELEASE_BRIEFING.highlights) assert.ok(body.includes(highlight), highlight);
...
assert.equal(modal.title, "What’s New & Feature Beacon · v10.3.9");
assert.equal(modal.subtitle, "Stable map colours during movement");
...
assert.deepEqual(opened[0].args, [
  "https://github.com/Conroy1988/missionchief-toolkit-assets/releases/tag/v10.3.9",
  "_blank",
  "noopener,noreferrer",
]);
