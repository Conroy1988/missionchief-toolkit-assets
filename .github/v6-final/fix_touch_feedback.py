...
ANCHOR = 'html[data-mcms-keyboard-open="true"] #${SCRIPT.panelId}'
...
def main() -> int:
    ...
    if TOUCH_TOKEN not in source:
        if source.count(ANCHOR) != 1:
            raise SystemExit(f"Expected one keyboard geometry anchor, found {source.count(ANCHOR)}")
        source = source.replace(ANCHOR, TOUCH_RULE + ANCHOR, 1)
    ...
    baseline = re.sub(
        ...
        f"| Source lines | 31,761 | {source_lines:,} | {signed(line_delta)} ({signed_pct(line_delta / baseline_lines * 100)}) |",
        baseline,
    )
