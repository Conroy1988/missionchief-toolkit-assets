...
assert.equal(metadataVersion, "10.16.7");
...
assert.match(body, /<strong>v10\.16\.7<\/strong>/u);
assert.match(body, /First-Byte UI Recovery and Direct Updates/u);
...
assert.equal(modal.title, "What’s New & Feature Beacon · v10.16.7");
assert.equal(modal.subtitle, "First-Byte UI Recovery and Direct Updates");
...
assert.deepEqual(opened[0].args, [
  "https://github.com/Conroy1988/missionchief-toolkit-assets/releases/tag/v10.16.7",
  "_blank",
  "noopener,noreferrer",
]);
