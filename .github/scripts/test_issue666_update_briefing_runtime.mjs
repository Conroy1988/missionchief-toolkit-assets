...
assert.equal(metadataVersion, "10.16.3");
...
assert.match(body, /<strong>v10\.16\.3<\/strong>/u);
assert.match(body, /Dispatch Recruitment Mismatch Selection/u);
...
assert.equal(modal.title, "What’s New & Feature Beacon · v10.16.3");
assert.equal(modal.subtitle, "Dispatch Recruitment Mismatch Selection");
...
assert.deepEqual(opened[0].args, [
  "https://github.com/Conroy1988/missionchief-toolkit-assets/releases/tag/v10.16.3",
  "_blank",
  "noopener,noreferrer",
]);
