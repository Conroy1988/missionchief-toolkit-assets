...
def main():
 manifest=json.loads((ROOT/'dist/release-manifest.json').read_text());dashboard=json.loads((ROOT/'status/release-dashboard.json').read_text());readme=(ROOT/'README.md').read_text();help_text=(ROOT/'help/index.html').read_text();greasy=(ROOT/'docs/greasyfork-description.md').read_text();hero=(ROOT/'docs/media/readme-hero.svg').read_text();site=json.loads((ROOT/'docs/site-data.json').read_text())
